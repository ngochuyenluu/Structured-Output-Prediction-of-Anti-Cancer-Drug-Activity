...
obj = 0; primal_ub = Inf; iter = 0; opt_round = 1;
profile_update; prev_obj = 0;
...
while or(primal_ub - obj > params.epsilon*obj,profile.n_err_microlbl ~= profile.n_err_microlbl_prev)
  ...
  for x = 1:m
    ...
    %Kmu = reshape(compute_Kmu(Kx_tr),4*size(E,1),m);
    %Kmu_x = Kmu(:,x);
    ...
    iter = iter + x_iter; profile.iter = iter; 
...
function [Ymax,Gmax] = max_gradient_labeling(gradient,max_iter)
...
function [Ymax,Gmax] = max_gradient_labeling_brute_force(gradient)

global E;
global l;
global Y_tr;

m = numel(gradient)/(size(E,1)*4);

Yall = zeros(2.^l,l);

for column = 1:l
    Yall(:,column) = repmat([-ones(2.^(column-1),1);ones(2.^(column-1),1)],2.^l/2.^column,1);    
end
Yall = fliplr(Yall);

Uall = reshape(Y2U(Yall),4*size(E,1),2.^l);
gradient = reshape(gradient,size(E,1)*4,m);

[Gmax,Imax] = max(gradient'*Uall,[],2);
Ymax = Yall(Imax,:);
...
U = zeros(4,size(E,1)*m);

U(1,:) = reshape(and(Y(:,E(:,1)) == -1,Y(:,E(:,2)) == -1)',1,size(E,1)*m);
U(2,:) = reshape(and(Y(:,E(:,1)) == -1,Y(:,E(:,2)) == 1)',1,size(E,1)*m);
U(3,:) = reshape(and(Y(:,E(:,1)) == 1,Y(:,E(:,2)) == -1)',1,size(E,1)*m);
U(4,:) = reshape(and(Y(:,E(:,1)) == 1,Y(:,E(:,2)) == 1)',1,size(E,1)*m);
...
while LBP_iter <= size(E,1)
    ...
    [Ymax,G] = max_gradient_labeling(gradient,LBP_iter); 
    Gmax = max(Gmax,G);
...
profile.start_time = cputime; profile.next_profile_tm = profile.start_time;
profile.n_err = 0; profile.p_err = 0; 
profile.n_err_microlbl = 0; profile.p_err_microlbl = 0; 
...
m = size(Ye,2);
  ...
  print_message(sprintf('alg: M3LBP tm: %d  iter: %d obj: %f mu: max %f min %f dgap: %f',...
    round(tm-profile.start_time),profile.iter,obj,max(max(mu)),min(min(mu)),primal_ub-obj),2,sprintf('%s.log',params.filestem));
  
  if params.profiling
    if tm > profile.next_profile_tm
      ...
      save(sfile,'Ypred_tr','Ypred_ts','params');
      Ye = reshape(Ye,4*size(E,1),m);
    end
  end
...
function MAdjE = buildEdgeAdjacencyMatrix(E)

MAdjE =  zeros(size(E,1));

for node = 1:max(max(E))
      meetingEdges = unique([find(E(:,1) == node),find(E(:,2) == node)]);
      MAdjE(meetingEdges,meetingEdges) = 1;   
end
...
function Ypred = compute_error(Y,Kx) 
...
Ypred = max_gradient_labeling(w_phi_e);
...
Ye_siz = size(Ye); Ye = reshape(Ye,4,size(E,1)*m);   
mu_siz = size(mu); mu = reshape(mu,4,size(E,1)*m);
m_oup = size(Kx,2);
